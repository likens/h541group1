...
class LifeLineMain extends Component {
	constructor() {
		...
		this.setInstructions(LifeLineApps.structure[0].name, LifeLineApps.structure[0].description, LifeLineApps.structure[0].interactions);
	}
	...
	goToApp(key) {
		...
		this.setInstructions(
			LifeLineApps.structure.find((app) => app.key === key).name,
		...
		return (
			<div className="main">
				<div className="container">
					<div className="nav__item nav__item--prev" onClick={() => this.goToApp(this.getAppKey(false))}>
					...
					<div className="nav__item nav__item--next" onClick={() => this.goToApp(this.getAppKey(true))}>
						...
						<div className={`notify notify--simple${this.state.notification ? ` notify--active` : ``}`}>
							<div className="notify__icon">
								{this.state.notification ? (
									<FontAwesomeIcon size={"3x"} icon={LifeLineNotifications.find((notif) => notif.key === this.state.notification)?.icon} />
								) : (
									``
								)}
							</div>
							...
							<div className="notify__close">
								<button
									className="notify__close-btn"
									onClick={() => this.removeNotification(LifeLineNotifications.find((notif) => notif.key === this.state.notification)?.key, false)}
								>
									<FontAwesomeIcon size={"2x"} icon={faTimes} />
								</button>
							</div>
											...
											{menu.menu.structure.map((item, i) => {
												return (
													<li key={i} className="menu__item">
														<button className={`menu__link${item.extra ? ` menu__link--extra` : ``}`} onClick={() => this.menuItemClick(item)}>
															{item.icon ? (
															...
															{menu.menu.key === KEY_EMERGENCY ? (
																<span className="menu__link-extra">
																	<FontAwesomeIcon size="lg" icon={item.key === this.state.submenu ? faCaretDown : faCaretRight} />
																</span>
															) : (
																``
															)}
															{menu.menu.key === KEY_SETTINGS ? (
																<span className="menu__link-extra">
																	{item?.sub?.length ? (
																		<FontAwesomeIcon size="lg" icon={item.key === this.state.submenu ? faCaretDown : faCaretLeft} />
																	) : (
																		``
																	)}
																</span>
														...
														<ul className={`menu__sub${item.key === this.state.submenu ? ` menu__sub--active` : ``}`}>
															...
															{menu.menu.key === KEY_EMERGENCY && item.edit ? (
																<li className="menu__item">
																	<button className="menu__link" onClick={() => this.toggleModal("edit")}>
																		<span className="menu__link-icon">
															...
															{menu.menu.key === KEY_EMERGENCY && item.delete ? (
																<li className="menu__item">
																	<button className="menu__link" onClick={() => this.toggleModal("delete")}>
																		<span className="menu__link-icon">
															...
															{item.sub?.map((sub, i) => {
																return (
																	<li key={i} className="menu__item">
																		<button className="menu__link" onClick={() => this.toggleModal(sub.modal)}>
										...
										{LifeLineForms.find((form) => form?.key === this.state.form)?.fields.map((field, i) => {
											return (
												<div key={i} className="field">
													...
													field.type === "label" ||
													field.type === "button" ? (
														<label htmlFor={i} className="field__label">
															{field.label}
														</label>
													) : (
														``
															...
															{field.options.map((o, i) => {
																return (
																	<Fragment>
																		...
																		<label htmlFor={field.label + i} key={i} className="field__radio-lbl">
																			{o}
																		</label>
																	</Fragment>
																);
							...
							<div className="modal__footer">
								{LifeLineModals.find((modal) => modal.key === this.state.modal)?.cancel ? (
									<button className="modal__btn modal__cancel" onClick={() => this.toggleOverlay()}>
										<span className="modal__btn-icon">
					...
					<div
						className={`main__overlay${this.state.menu || this.state.modal || this.state.notifications.center ? ` main__overlay--active` : ``}`}
